...
```
> :information_source: For security reasons, it's essential to use files to ingest private information instead of relying on command line input. In a production environment, it's important to set the appropriate permissions for the private file "key.store." This can be done by setting the file permission to "600," which restricts read and write access to the file to its owner only.
